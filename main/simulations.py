...
# -- Start --
def simulation_mt_without_network(power_pontas, power_cheias, power_vazio_normal, power_super_vazio, price_p_month):
    lowest_value = sys.float_info.max
...
def simulation_mt_with_network(power_pontas, power_cheias, power_vazio_normal, power_super_vazio, price_p_month):
    lowest_value = sys.float_info.max
    for x in Tariff_MT.objects.all():
        value = 0.0
        if django.utils.timezone.now > x.mt_tariff_date_start and django.utils.timezone.now > x.mt_tariff_date_end:
...
# -- Start --
def simulation_bte_with_network(power_pontas, power_cheias, power_vazio_normal, power_super_vazio, price_p_month):
    lowest_value = sys.float_info.max
...
def simulation_bte_without_network(power_pontas, power_cheias, power_vazio_normal, power_super_vazio, price_p_month):
    lowest_value = sys.float_info.max
    for x in Tariff_BTE.objects.all():
        value = 0.0
        if django.utils.timezone.now > x.bte_tariff_date_start and django.utils.timezone.now > x.bte_tariff_date_end:
